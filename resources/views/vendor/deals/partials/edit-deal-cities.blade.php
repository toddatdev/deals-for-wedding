...
@section('title', 'Advertiser: Edit Deals')
                ...
                <div class="row">
                    <div class="col-md-12">
                        <div style="padding: 3% 1% 0 1% !important;" class="card">
                            <h2 id="heading" class="font-weight-bold">Add Additional City</h2>
                            <section class="wizard-section">
                                <div class="row no-gutters">
                                    <div class="col-lg-12 col-md-12">
                                        <div class="form-wizard">
                                            <form action="{{route('vendor.deals.update', $deal->id)}}" method="post"
...
{{--                                                    </ul>--}}
                                                ...
                                                <fieldset class="wizard-fieldset d-none">
                                                    <h5>Service & City</h5>
                                                        ...
                                                        <div class="form-group">
                                                            ...
                                                            <div class="row" style="margin-top:4%;">
                                                                ...
                                                                <div class="col-md-6">
                                                                    ...
                                                                    <select
                                                                            class="form-control editCity wizard-required @error('state_id') is-invalid @enderror"
                                                                ...
                                                                @if(isset($deal->image))
                                                                    <div class="old_img">
                                                                        <img src="{{asset($deal->image)}}"
                                                                             style="width:100px">
                                                                        <i class="fa fa-times text-success" aria-hidden="true"
                                                                           onclick="removeFile(this);">Remove</i>
                                                                    </div>
                                                                @endif
                                                                <input id="imgInp" type="file" id="image" name="image"
                                                    ...
                                                    <div class="form-group">
                                                        <div class="row">
                                                            ...
                                                            <div class="col-md-6">
                                                                ...
                                                                @php
                                                                    $Existing_cities = 0;
                                                                        if(!is_null($deal->multiple_cities) ){
                                                                         $cities = json_decode($deal->multiple_cities);
                                                                         if (is_array($cities)){
                                                                             $Existing_cities = count($cities);
                                                                         }
                                                                ...
                                                                <select
                                                                    ...
                                                                    @foreach($states as $state)
                                                                        <option value="{{isset($state->id) ? $state->id : ''}}" @if ($deal->multiple_cities != 'null' && $deal->multiple_cities != 'NULL' && !empty($deal->multiple_cities))
...
{{--                                                           class="form-wizard-previous-btn float-left">Previous</a>--}}
                                                        <button type="submit" href="javascript:;"
                                                                ...
                                                                style="margin: 0 15px">Update Additonal City</button>
...
@push('customJs')
    ...
    <script>
        $(function () {
            ...
            {{--console.log({{$checkout->additional_city_price}});--}}
            ...
            $('.getSelectedOptionCount').change(function () {
                ...
                if (count > 0) {
                    ...
                    let existingCities = {{$Existing_cities}};

                    if(count > existingCities) {
                        let currentCity = count - existingCities;
                        let total = currentCity*CityPrice;
    ...
    <script>
        $(function () {
            ...
            @if(is_array(json_decode($deal->multiple_cities)))
                @foreach(json_decode($deal->multiple_cities) as $city)
                multiSelectedCities.push(
                        {{isset($city) ? $city : ''}}
                );
                @endforeach
            @endif
            @endif
            let selCity = $('.editCity').find(':selected').val();
            $('#state_mid').html('');
            multiCities.filter(mc => parseInt(mc.value) !== parseInt(selCity))
                .map(mc => {
                    if( multiSelectedCities.includes(parseInt(mc.value))) {
                        $('#state_mid').append(`<option value="${mc.value}" selected>${mc.text}</option>`);
                    } else {
    ...
    <script>
        $('body').on('click keyup change',  function(){
            if ($('#state_mid :selected').length > 0) {
                ...
                var existingCities = {{$Existing_cities}};
                totalCity = parseFloat($('#state_mid :selected').length);
                if(totalCity > existingCities){
                    var currentCities = totalCity - existingCities;
                    $('#checkout-price').val(cityPrice*currentCities);
                }
