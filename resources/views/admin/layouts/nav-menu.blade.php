...
<div class="main-header">
	<div class="logo-header" style="background-color: #636680;padding: 0 10px;">
		<div class="">
						...
						<li>
							<div class="notif-center">
								...
								<a id="notify_{{$notification->id}}" class="notifyaccess" href="javascript:;">
									<div style="width: 50px" class="notif-icon notif-success"> <i class="la la-comment"></i> </div>
									<div style="flex-basis: 95%" class="notif-content">
										<span class="block">
											{!! $notification->data['body'] !!}
										</span>
										{{-- <span class="time"><a href="">Mark as read!</a></span> --}}
									</div>
								</a>
								<script>
									setTimeout(() => {
...
<li>
	<a class="see-all" href="{{route('admin.notifications')}}"> <strong>See all notifications</strong> <i class="la la-angle-right"></i> </a>
</li>
</ul>
</li>
	...
	<ul class="dropdown-menu dropdown-user">
		<li>
			<div class="user-box">
				<div class="u-img">
					@if(isset($adminDetails->userDetails->image))
					<img src="{{asset($adminDetails->userDetails->image)}}" alt="user-img" width="36" class=""><span>{{ $adminDetails->fname }} {{ $adminDetails->lname }}</span>
					@else
					<img src="{{asset('admin/img/profile.jpg')}}" alt="user-img" width="36" class=""><span>{{ $adminDetails->fname }} {{ $adminDetails->lname }}</span>
					@endif
				</div>
				<div class="u-text">
					<h4>{{ $adminDetails->fname }} {{ $adminDetails->lname }}</h4>
					<p class="text-muted">{{ $adminDetails->email }}</p><a href="{{route('admin.profile')}}" class="btn btn-rounded btn-danger btn-sm">View Profile</a>
				</div>
			</div>
		</li>
		<div class="dropdown-divider"></div>
